const HREditor = (() => {
    ...
    const defaultConfig = {
        ...
        translations: {
            ...
            enterVideoURL: 'Enter the video URL',
            enterNewWidth: 'Enter new width',
            enterMargin: 'Enter margin to',
            uploadImage: 'Upload Image', 
            insertImageURL: 'Insert Image URL', 
            uploadVideo: 'Upload Video', 
            insertVideoURL: 'Insert Video URL' 
        }
    };
    ...
    const toolbarButtons = {
        ...
        insertImage: (translations) => `<div class="dropdown">
                         <button type="button" id="imageButton"><i class="fas fa-image"></i></button>
                         <div class="dropdown-content">
                             <div class="content-inner">
                                 <button type="button" id="imageUploadButton" value="Upload Image">${translations.uploadImage}</button>
                                 <button type="button" id="imageUrlButton" value="Insert Image">${translations.insertImageURL}</button>
                                 <input type="file" id="imageUpload" style="display:none;" accept="image/*">
                             </div>
                         </div>
                      </div>`,
        insertVideo: (translations) => `<div class="dropdown">
                         <button type="button" id="videoButton"><i class="fas fa-video"></i></button>
                         <div class="dropdown-content">
                             <div class="content-inner">
                                 <button type="button" id="videoUploadButton" value="Upload Video">${translations.uploadVideo}</button>
                                 <button type="button" id="videoUrlButton" value="Insert Video">${translations.insertVideoURL}</button>
    ...
    function createToolbar(config) {
        ...
        (config.toolbar || defaultConfig.toolbar).forEach(button => {
            toolbar.innerHTML += (toolbarButtons[button] ? toolbarButtons[button](translations) : '');
        });
        return toolbar;
    }

    function handleToolbarButtonClick(button, translations) {
        const command = button.getAttribute('data-command');
        if (commandsWithPrompts[command]) {
            const promptMessage = translations[commandsWithPrompts[command]] || 'Enter the value';
    ...
    function init(container, config = {}) {
        ...
        toolbar.querySelectorAll('button[data-command]').forEach(button => {
            button.addEventListener('click', () => handleToolbarButtonClick(button, translations));
        });
        ...
        const handleUrlInsert = (type, translations) => {
            const url = prompt(translations[`enter${type.charAt(0).toUpperCase() + type.slice(1)}URL`]);
            if (url) {
        ...
        toolbar.querySelector('#imageUrlButton').addEventListener('click', () => handleUrlInsert('img', translations));
        ...
        toolbar.querySelector('#videoUrlButton').addEventListener('click', () => handleUrlInsert('video', translations));
        ...
        function addMarginToElement(element, direction) {
            ...
            const newMargin = prompt(`${translations.enterMargin} ${direction} (px or %)`, style[currentMargin]);
            if (newMargin) {
        ...
        imageToolbar.querySelectorAll('button').forEach(button => {
            button.addEventListener('click', (event) => {
                ...
                if (id.includes('float')) {
                    ...
                    const newSize = prompt(translations.enterNewWidth, target.width || target.style.width);
                    if (newSize) {
